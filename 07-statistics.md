...
prob_frat_given_ident = 1.0

prob_ident_given_frat = (prob_frat_given_ident * prob_ident) / prob_frat

print prob_ident_given_frat
```

> Result: **0.416666666667**


> There's a 41.67% chance that Elvis was an identical twin given that he was a twin.
...
> In Frequentist statistics, there are no probabilities, just long-run frequencies. We must treat all events as repeated events -- even events which can only happen once, like the 2016 US Presidential election. In this case, we have to imagine that the same election would happen multiple times in a sort of "parallel universes" construct so that we can determine the "frequency" of this one-time event.

> Bayesian statistics is more like the natural intuitive idea of "probability" as used in plain language: the probability of an even is the likelihood that it will occur, given prior information and acknowledging uncertainty about the outcome.
